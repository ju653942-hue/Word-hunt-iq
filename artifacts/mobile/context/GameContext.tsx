...
function pickWords(allWords: string[], count: number, difficulty: Difficulty): string[] {
  ...
  // Pass 3× the needed words so generatePuzzle always has enough candidates
  // to guarantee exactly `count` words are placed, even when some don't fit.
  const poolSize = Math.min(count * 3, pool.length);
  return pool.sort(() => Math.random() - 0.5).slice(0, poolSize);
}
...
function progressKey(cat: Category, diff: Difficulty): string {
  return `${cat}_${diff}`;
}
...
export function GameProvider({ children }: { children: React.ReactNode }) {
  ...
  const claimDailyReward = useCallback(() => {
    ...
    const newHints = hints + reward.hints;
    setCoins(newCoins);
    setHints(newHints);
  ...
  const startGame = useCallback((daily = false) => {
    ...
    // Per-category+difficulty progress: resume from where user left off
    const pKey = progressKey(cat, diff);
    const savedIdx = !daily ? categoryProgress[pKey] : undefined;
    // Validate saved index belongs to correct category+difficulty
    const savedLevel = savedIdx != null ? LEVEL_ORDER[savedIdx] : null;
    const isValidSaved = savedLevel != null &&
      savedLevel.category === cat &&
      savedLevel.difficulty === diff;
    const targetIdx = isValidSaved ? (savedIdx as number) : getStartingLevelIdx(cat, diff);
    setCurrentLevelIdx(targetIdx);
    setIsDaily(daily);

    // Update categoryProgress and persist
    const newProgress = { ...categoryProgress, [pKey]: targetIdx };
    setCategoryProgress(newProgress);
    savePlayer({
      currentLevelIdx: targetIdx,
      savedCategory: cat,
    ...
    // InteractionManager.runAfterInteractions doesn't fire on web — use setTimeout instead
    setTimeout(() => {
      const newPuzzle = generatePuzzle(selected, config.gridSize, diff, wordCount);
  ...
  }, [category, difficulty, currentLevelIdx, categoryProgress, savePlayer]);
  ...
  const endGame = useCallback(
    async (won: boolean, finalScore: number, foundCount: number, currentCombo: number) => {
      ...
      if (lastPlayedDate !== today) {
        ...
        newStreak = lastPlayedDate === yesterdayStr ? streak + 1 : 1;
        setStreak(newStreak);
    ...
    [score, timeLeft, highScores, category, difficulty, xp, coins, streak, lastPlayedDate,
  ...
  const startNextLevel = useCallback(() => {
    ...
    // Save progress for the next category+difficulty so it resumes correctly
    const pKey = progressKey(nextCat, nextDiff);
    const newProgress = { ...categoryProgress, [pKey]: nextIdx };
    setCategoryProgress(newProgress);
    savePlayer({
      currentLevelIdx: nextIdx,
      savedCategory: nextCat,
      savedDifficulty: nextDiff,
      categoryProgress: newProgress,
    } as Partial<PlayerData>);
    ...
    InteractionManager.runAfterInteractions(() => {
      const newPuzzle = generatePuzzle(selected, config.gridSize, nextDiff, wordCount);

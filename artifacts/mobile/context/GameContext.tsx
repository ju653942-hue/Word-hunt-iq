...
export function GameProvider({ children }: { children: React.ReactNode }) {
  ...
  const startGame = useCallback((daily = false) => {
    ...
    void (async () => {
      ...
      try {
        const newPuzzle = await Promise.resolve().then(() =>
          generatePuzzle(selected, config.gridSize, diff, wordCount)
        );
  ...
  const startNextLevel = useCallback(() => {
    ...
    void (async () => {
      ...
      try {
        const newPuzzle = await Promise.resolve().then(() =>
          generatePuzzle(selected, config.gridSize, nextDiff, wordCount)
        );
        if (generationId !== generationIdRef.current) return;
        ...
        console.error("Next-level puzzle generation failed:", error);
        if (generationId === generationIdRef.current) {
          setIsPlaying(false);
        }
      } finally {

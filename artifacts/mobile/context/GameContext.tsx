...
export function GameProvider({ children }: { children: React.ReactNode }) {
  ...
  const startGame = useCallback((daily = false) => {
    ...
    const newPuzzle = generatePuzzle(allWords, config.gridSize, diff, wordCount);

    const newBonus = diff === "medium" && newPuzzle.placedWords.length > 0
      ? newPuzzle.placedWords[Math.floor(Math.random() * newPuzzle.placedWords.length)].word
      : null;
  ...
  const startNextLevel = useCallback(() => {
    ...
    const newPuzzle = generatePuzzle(allWords, config.gridSize, nextDiff, wordCount);

    const newBonus = nextDiff === "medium" && newPuzzle.placedWords.length > 0
      ? newPuzzle.placedWords[Math.floor(Math.random() * newPuzzle.placedWords.length)].word
      : null;
    setBonusWord(newBonus);

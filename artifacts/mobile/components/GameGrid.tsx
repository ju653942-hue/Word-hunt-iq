...
function getDifficultyTheme(difficulty: string): DifficultyTheme {
  switch (difficulty) {
    case "medium":
      return {
        gridBg:             "#1a0410",
        ...
        cellBorder:         "#c01850",
        cellText:           "#ffb3cc",
        wordColors:         ["#be185d", "#f43f5e"],
        wordTextColors:     ["#fce7f3", "#ffe4e6"],
        trailOuter:         "#f43f5e",
        trailInner:         "#fb7185",
        trailCore:          "#ffd6e0",
        selectedBg:         "#5c0a28cc",
        selectedBorder:     "#fb7185",
        lastSelectedBg:     "#7a0a30dd",
        lastSelectedBorder: "#ffd6e0",
      };
    case "hard":
      return {
        gridBg:             "#04130a",
        ...
        cellBorder:         "#059669",
        cellText:           "#a7f3d0",
        wordColors:         ["#059669", "#e11d48", "#db2777"],
        wordTextColors:     ["#d1fae5", "#ffe4e6", "#fce7f3"],
        trailOuter:         "#10b981",
        trailInner:         "#34d399",
        trailCore:          "#a7f3d0",
        selectedBg:         "#0a3d1add",
        selectedBorder:     "#34d399",
        lastSelectedBg:     "#0a4d20dd",
        lastSelectedBorder: "#a7f3d0",
      };
    default: // easy
      return {
        gridBg:             "#160630",
        ...
        cellBorder:         "#b87a0c",
        cellText:           "#ffe066",
        wordColors:         ["#7c3aed", "#d97706"],
        wordTextColors:     ["#e9d5ff", "#fef3c7"],
        trailOuter:         "#9b5fff",
        trailInner:         "#c084fc",
        trailCore:          "#ffe066",
        selectedBg:         "#250a60cc",
...
const SelectionTrail = memo(function SelectionTrail({
  ...
  const points = selectedCells
    ...
    .join(" ");
  return (
    <Svg style={[StyleSheet.absoluteFill, { pointerEvents: "none" } as any]}>
      <Polyline points={points} stroke={theme.trailOuter} strokeWidth={cellSize * 0.52} strokeLinecap="round" strokeLinejoin="round" fill="none" opacity={0.22} />
      <Polyline points={points} stroke={theme.trailInner} strokeWidth={cellSize * 0.28} strokeLinecap="round" strokeLinejoin="round" fill="none" opacity={0.45} />
      <Polyline points={points} stroke={theme.trailCore}  strokeWidth={cellSize * 0.09} strokeLinecap="round" strokeLinejoin="round" fill="none" opacity={0.85} />
    </Svg>
  );
});
...
const GridCell = memo(function GridCell({
  ...
  isHint, isStartCell, foundColorIdx, foundOrder, cellSize, theme,
}: GridCellProps) {
  ...
  if (isFound && foundColorIdx !== undefined) {
    bgColor     = theme.wordColors[foundColorIdx % theme.wordColors.length];
    borderColor = theme.wordColors[foundColorIdx % theme.wordColors.length];
    textColor   = theme.wordTextColors[foundColorIdx % theme.wordTextColors.length];
  } else if (isLastSelected) {
    bgColor     = theme.lastSelectedBg;
    borderColor = theme.lastSelectedBorder;
    borderWidth = 2.5;
    textColor   = "#ffffff";
  } else if (isSelected) {
  ...
  const glowColor = foundColorIdx !== undefined
    ? theme.wordColors[foundColorIdx % theme.wordColors.length]
    : theme.trailCore;
...
export default function GameGrid() {
  ...
  // Page coords at the moment of grant — used for delta tracking in MOVE events.
  const grantPageRef        = useRef({ x: 0, y: 0 });
  const grantCellRef        = useRef<CellCoord | null>(null);

  // Performance: RAF batching for setState during drag + tick-sound throttle
  const rafRef              = useRef<ReturnType<typeof requestAnimationFrame> | null>(null);
  const pendingSelectionRef = useRef<CellCoord[] | null>(null);
  const lastSoundTimeRef    = useRef<number>(0);
  const TICK_SOUND_MIN_MS   = 80;
  ...
  useEffect(() => {
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      // Throttle tick sound — at most once per TICK_SOUND_MIN_MS
      const now = Date.now();
      if (now - lastSoundTimeRef.current >= TICK_SOUND_MIN_MS) {
      ...
      if (lineCells) {
        ...
        pendingSelectionRef.current = [...lineCells];
        // Batch setState to one update per animation frame
        if (!rafRef.current) {
          rafRef.current = requestAnimationFrame(() => {
            rafRef.current = null;
            if (pendingSelectionRef.current) {
              setSelectedCellsRef.current(pendingSelectionRef.current);
            }
          });
        }
      }
  ...
  // Native PanResponder
  // PanResponder is placed on a transparent full-size overlay view (no children).
  // With no child views, locationX/locationY are always relative to the overlay itself
  // (= the grid container), not to an individual cell. No measureInWindow needed.
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderGrant: (evt) => {
        ...
        // The PanResponder is on the transparent overlay (no children), so
        // locationX/locationY are always relative to the overlay = grid container.
        // No measureInWindow needed — eliminates stale-origin offset bugs on Android.
      ...
      onPanResponderMove: (evt) => {
        ...
        const gSize = gridSizeRef.current;
        if (cSize === 0) return;
        // Delta from the initial touch in page coords → cell offset.
        // This works regardless of how locationX/locationY behave on Android
        // (child-view-relative vs responder-view-relative).
        const deltaX = pageX - grantPageRef.current.x;
        const deltaY = pageY - grantPageRef.current.y;
        const col = Math.max(0, Math.min(gSize - 1, startCell.col + Math.round(deltaX / cSize)));
        const row = Math.max(0, Math.min(gSize - 1, startCell.row + Math.round(deltaY / cSize)));
  ...
  return (
    <View key={puzzleKey} style={[styles.outerWrapper, { width: gridTotalSize }]}>
      <View
        ref={gridRef}
        style={[styles.container, { backgroundColor: theme.gridBg, borderColor: theme.gridBorder }]}
      >
        ...
        <SelectionTrail selectedCells={selectedCells} cellSize={cellSize} theme={theme} />
        <FloatingFeedback lastFoundCells={lastFoundCells} score={score} />
        <ParticleBurst trigger={particleTrigger} />
        {/* Transparent touch capture overlay — rendered last so it sits on top.
            Having no child views ensures locationX/locationY are relative to
            this overlay (= grid container), not to an individual GridCell.
            This eliminates the "touch-the-cell-below" offset on Android. */}
        {Platform.OS !== "web" && (
          <View

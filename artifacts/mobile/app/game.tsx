...
const DIFFICULTY_BG = {
  ...
  medium: require("@/assets/Images/bg_medium.jpg"),
  hard:   require("@/assets/Images/bg_hard.jpg"),
};
...
import { getDifficultyTheme } from "@/constants/difficultyThemes";
...
import { askForRating } from "@/utils/rating";
      ...
      if (didWin) {
        ...
        void (async () => {
          try {
            ...
            console.warn("Rating prompt failed:", error);
          }
        })();
      } else {
        playGameOverSound();
...
function WatchAdBar() {
  ...
  const handlePress = async () => {
    ...
    if (!ok) flashMsg("Ad unavailable. Please try again later.");
  };
...
export default function GameScreen() {
  ...
  const { puzzle, category, difficulty, hints, useHint, isPlaying, isGenerating, foundWords } = useGame();
  ...
  return (
    <ImageBackground
      source={DIFFICULTY_BG[difficulty]}
      style={styles.screen}
      resizeMode="cover"
    >
      {/* Light overlay — keeps UI readable while letting nature image show through */}
      <View style={[styles.bgOverlay, { backgroundColor: theme.screenBg + "55" }]} />
      ...
      >
        <ScaleButton
          ...
          style={[styles.headerBtn, {
            backgroundColor: theme.chipPendingBg,
            borderColor: theme.chipPendingBorder,
            shadowColor: "#000",
            shadowOpacity: 0.35,
            shadowRadius: 4,
        ...
        <Animated.View style={{ transform: [{ scale: hintScale }] }}>
          <Pressable
            ...
            style={[
              styles.headerBtn,
              {
                backgroundColor: hints > 0 && isPlaying ? theme.hintBg : theme.chipPendingBg,
                borderColor: hints > 0 && isPlaying ? theme.hintBorder : theme.chipPendingBorder,
                shadowColor: "#000",
                shadowOpacity: 0.3,
                shadowRadius: 4,
                shadowOffset: { width: 0, height: 2 },
      ...
      {/* Loading overlay while puzzle is being generated */}
      {isGenerating && (
        <View style={styles.generatingOverlay}>
          <ActivityIndicator size="large" color="#c8a8f0" />

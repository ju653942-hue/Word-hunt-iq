...
function attemptGeneratePuzzle(
  ...
  const candidates = words.filter(w => w.length <= gridSize);
...
export function generatePuzzle(
  ...
  // Filter words by difficulty word-length rules before retrying
  ...
  // This guarantees all words are placed when the grid has room.
  const maxAttempts = difficulty === "hard" ? 10 : 5;
  let best: Puzzle | null = null;

  for (let i = 0; i < maxAttempts; i++) {
    const attempt = attemptGeneratePuzzle(candidates, gridSize, difficulty);
    if (!best || attempt.placedWords.length > best.placedWords.length) {
      best = attempt;
    }
    // All words placed — no need to retry

...
function attemptGeneratePuzzle(
  ...
  // Ensure no word is longer than the grid, then sort longest-first
  const sorted = words
  ...
  for (const word of sorted) {
    // Stop as soon as we have exactly the required number of words
...
export function generatePuzzle(
  ...
  // targetCount is the exact number of words to place.
  // `words` should be a larger pool (e.g. 3× target) so we always have
  // enough candidates — this guarantees exactly targetCount words are placed.
  const target = targetCount ?? words.length;

  // Filter words by difficulty word-length rules
  let candidates = [...words];
  if (difficulty === "easy") {
    const short = candidates.filter(w => w.length <= EASY_MAX_WORD_LENGTH);
    if (short.length >= target) candidates = short;
  } else if (difficulty === "hard") {
    const long = candidates.filter(w => w.length >= HARD_MIN_WORD_LENGTH);
    if (long.length >= target) candidates = long;
  }

  // Retry up to 10 times — keep the attempt that placed the most words.
  const maxAttempts = 10;
  let best: Puzzle | null = null;

  for (let i = 0; i < maxAttempts; i++) {
    const attempt = attemptGeneratePuzzle(candidates, gridSize, difficulty, target);
    if (!best || attempt.placedWords.length > best.placedWords.length) {
      best = attempt;
    }
    // Reached exact target — done
